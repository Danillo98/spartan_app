-- FIX FINAL V5 - MODO CIRÚRGICO
-- O problema é uma Trigger antiga em auth.users competindo com nossa RPC.
-- Vamos desabilitar triggers temporariamente dentro da transação para garantir que SÓ NÓS façamos o insert.
...
BEGIN
    ...
    -- ---------------------------------------------------------
    -- TRUQUE DE MESTRE: Desabilitar Triggers para esta sessão
    -- Isso impede que triggers antigas tentem inserir user_alunos duplicado ou errado
    -- ---------------------------------------------------------
    ...
    -- 2. Insert Público (Manual e Correto)
...
EXCEPTION WHEN OTHERS THEN
    -- Garantir que voltamos ao normal mesmo em erro
    SET LOCAL session_replication_role = 'origin';
    RETURN jsonb_build_object('success', false, 'message', 'ERRO SQL (V5): ' || SQLERRM);
END;
$$;
