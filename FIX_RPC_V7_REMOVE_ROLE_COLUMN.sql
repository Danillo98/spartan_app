-- FIX FINAL V7 - REMOVENDO COLUNA INEXISTENTE 'ROLE'
-- O erro anterior confirmou que a tabela users_alunos NÃO TEM colunas 'role'.
-- Vamos remover essa coluna dos inserts.
...
BEGIN
    ...
    -- 2. Insert Público (SEM A COLUNA ROLE)
    IF v_role = 'student' THEN
        INSERT INTO public.users_alunos (
            id, id_academia, created_by_admin_id, nome, email, telefone, academia, cnpj_academia, is_blocked, payment_due_day
        ) VALUES (
            v_user_id, v_id_academia, (p_metadata->>'created_by_admin_id')::UUID, p_metadata->>'name', p_email, p_metadata->>'phone', v_academia_nome, v_cnpj_academia, false, (p_metadata->>'paymentDueDay')::INTEGER
        );
    ELSIF v_role = 'trainer' THEN
         INSERT INTO public.users_personal (
            id, id_academia, created_by_admin_id, nome, email, telefone, academia, cnpj_academia, is_blocked
        ) VALUES (
            v_user_id, v_id_academia, (p_metadata->>'created_by_admin_id')::UUID, p_metadata->>'name', p_email, p_metadata->>'phone', v_academia_nome, v_cnpj_academia, false
        );
...
EXCEPTION WHEN OTHERS THEN
    RETURN jsonb_build_object('success', false, 'message', 'ERRO SQL (V7): ' || SQLERRM);
END;
$$;
